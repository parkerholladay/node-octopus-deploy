...
const promoteRelease = async (simpleReleaseParams, simpleDeployParams) => {
  const { projectSlugOrId, version } = simpleReleaseParams
  const { environmentName } = simpleDeployParams
  ...
  const [release, deployParams] = await Promise.all([
    api.projects.getReleaseByProjectAndVersion(project.value.id, version),
    paramsBuilder.buildDeployParams(simpleDeployParams, project.value)
  ])

  if (!release.hasValue || !deployParams.hasValue) {
    return Maybe.none()
  }
