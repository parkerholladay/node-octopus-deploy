...
function getVariableId(variableSet, variableName, environmentId, environmentName) {
  const foundVariables = variableSet.Variables.filter(varWithScope => {
  ...
  return getProject.execute(projectSlugOrId)
    ...
    .then(variableSet => {
      ...
      variableNames.map(variableName => {
        const variableId = getVariableId(variableSet, variableName, environment.Id, environmentName)
