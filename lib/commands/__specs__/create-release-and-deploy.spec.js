...
describe('commands/create-release-and-deploy', () => {
  ...
  afterEach(() => {
    ...
    octo.api.deployments.create.restore()
  })
  ...
  it('deploys the release', () => {
    ...
    sinon.stub(octo.api.deployments, 'create').callsFake(() => BPromise.resolve(deployment))

    return subject(releaseParams, deployParams).then(actual => {
      expect(actual).to.eql(deployment)
      expect(octo.api.releases.create).to.be.calledWith(releaseParams)
      return expect(octo.api.deployments.create).to.be.calledWith(release.id, deployParams)
    })
  })
  ...
  describe('when create release fails', () => {
    it('does not attempt to deploy', () => {
      ...
      sinon.stub(octo.api.deployments, 'create').callsFake(() => BPromise.reject('fail deploy'))

      return subject(releaseParams, deployParams).catch(err => {
        expect(octo.api.deployments.create).to.not.be.called
  ...
  describe('when deploy fails', () => {
    it('rejects with an error', () => {
      sinon.stub(octo.api.releases, 'create').callsFake(() => BPromise.resolve(createRelease()))
      sinon.stub(octo.api.deployments, 'create').callsFake(() => BPromise.reject('fail deploy'))
