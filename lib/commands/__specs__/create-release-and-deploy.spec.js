...
describe('commands/create-release-and-deploy', () => {
  ...
  describe('when create release fails', () => {
    it('does not attempt to deploy', () => {
      sinon.stub(octopusApi.releases, 'create').callsFake(() => BPromise.reject('fail'))
      sinon.stub(octopusApi.deployments, 'create').callsFake(() => BPromise.reject('fail'))

      return subject(releaseParams, deployParams).catch(_ => {
        return expect(octopusApi.deployments.create).to.not.be.called
      })
    })
  })
})
