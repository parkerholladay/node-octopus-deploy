...
const { generateDeploymentProcess, generateProject, generateVariableSet } = require('../../../test/mocks')
...
describe('commands/simple-create-release-and-deploy', () => {
  const project = generateProject({ id: 'HotS', slug: 'heroes-of-the-storm' })
  ...
  describe('when project exists', () => {
    const deployment = generateDeploymentProcess({ projectId: project.id, environmentId: environment.id, formValues, specificMachineIds: machineIds, comments })
  ...
  describe('when selecting packages fails', () => {
    it('does not attempt to get variables', () => {
      ...
      sandbox.stub(octopus.variables, 'find')
  ...
  describe('when variable set does not exist', () => {
    it('does not attempt to get environment', () => {
      ...
      sandbox.stub(getEnvironmentId, 'execute')

      return subject(simpleReleaseParams, simpleDeployParams).catch(err => {
        expect(getEnvironmentId.execute).to.not.be.called
        return expect(err).to.equal('fail variables')
      })
    })
  })
  ...
  describe('when environment is not found', () => {
    it('does not attempt to get form values', () => {
      ...
      sandbox.stub(getFormValues, 'execute')

      return subject(simpleReleaseParams, simpleDeployParams).catch(err => {
        expect(getFormValues.execute).to.not.be.called
        return expect(err).to.equal('fail environment')
      })
    })
  })
  ...
  describe('when creating form values fails', () => {
    it('does not attempt to create release and deploy', () => {
      ...
      sandbox.stub(createReleaseAndDeploy, 'execute')

      return subject(simpleReleaseParams, simpleDeployParams).catch(err => {
        expect(createReleaseAndDeploy.execute).to.not.be.called
        return expect(err).to.equal('fail formValues')
      })
    })
  })
  ...
  describe('when creating release and deploying fail', () => {
    it('rejects with an error', () => {
      ...
      return expect(subject(simpleReleaseParams, simpleDeployParams)).to.eventually.be.rejectedWith('fail deploy')
    })
  })
})
