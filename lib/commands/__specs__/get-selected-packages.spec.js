...
describe('commands/get-selected-packages', () => {
  ...
  afterEach(() => {
    sinon.restore(octopusApi.processes.find)
  })

  describe('when the deployment process has steps', () => {
    it('returns packages with versions', () => {
      sinon.stub(octopusApi.processes, 'find', () => BPromise.resolve(mockProcess))
  ...
  describe('when the deployment process does not have steps', () => {
    it('returns nothing', () => {
      ...
      sinon.stub(octopusApi.processes, 'find', () => BPromise.resolve(processWithNoSteps))
