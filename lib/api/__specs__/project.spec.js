...
describe('api/project', () => {
  ...
  describe('#find', () => {
    beforeEach(() => {
      sinon.stub(client, 'get', () => BPromise.resolve(mockProject))
    })
    ...
    it('finds a project by id', () => {
      ...
      return subject.find(projectId).then(project => {
        ...
        expect(client.get).to.be.calledWith(`/projects/${projectId}`)
      })
    })
    ...
    it('finds a project by slug', () => {
      ...
      return subject.find(slug).then(project => {
        expect(project).to.eql(mockProject)
        expect(client.get).to.be.calledWith(`/projects/${slug}`)
      })
    })
  })
  ...
  describe('#getReleasesByProject', () => {
    it('returns releases', () => {
      const projectId = Math.floor(Math.random() * 100)
      sinon.stub(client, 'get', () => BPromise.resolve([mockProjectRelease]))
