...
describe('api/release', () => {
  describe('#find', () => {
    ...
    it('finds a release', () => {
      ...
      sinon.stub(client, 'get').callsFake(() => BPromise.resolve(mockRelease))

      return subject.find(releaseId).then(release => {
        expect(release).to.eql(mockRelease)
        return expect(client.get).to.be.calledWith(`/releases/${releaseId}`)
      })
    })
  })
  ...
  describe('#create', () => {
    ...
    it('creates a release', () => {
      ...
      const releaseParams = { projectId, releaseNotes, version, selectedPackages }
      sinon.stub(client, 'post').callsFake(() => BPromise.resolve(mockRelease))
