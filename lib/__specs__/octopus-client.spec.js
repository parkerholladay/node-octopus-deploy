...
const { getApiConfig, setApiConfig, logger } = require('../utils')
...
describe('octopus-client', () => {
  ...
  describe('#get', () => {
    ...
    describe('when request spaceId is set', () => {
      it('appends space to baseUrl', async () => {
        const data = { Kerrigan: 'Queen of Blades', Raynor: 'Raynor\'s Raiders' }
        const expected = JSONCasing.toCamel(data)
        const spaceId = 'Spaces-22'
        sandbox.stub(request, 'get').resolves(data)
        ...
        expect(actual.value).to.deep.equal(expected)
        expect(request.get).to.have.been.calledWith(sinon.match({ baseUrl: '#{octopusApiHost}/api/Spaces-22' }))
      })
    })
